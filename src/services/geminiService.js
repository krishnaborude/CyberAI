...
class GeminiService {
  ...
  validateStudyPlanCompleteness(text, { expectedWeeks = null, certification = '', focusArea = '' } = {}) {
    const issues = [];
    ...
    if (cert) {
      ...
      const alignmentText = `${alignmentNotes}\n${text}`;
      if (!/\b(exam|assessment|scoring|deliverable|report|methodology|workflow)\b/i.test(alignmentText)) {
        issues.push('Certification Alignment Notes should explicitly connect to exam style, assessment, and reporting expectations.');
      }
    }

    const focus = String(focusArea || '').toLowerCase().trim();
    const weeklyBody = getSectionBody('Weekly Breakdown').toLowerCase();
    if (focus && expectedWeeks && weeklyBody) {
      const terms = [];
      ...
      const weekLines = weeklyBody.split('\n').filter((line) => /\bweek\s+\d+\b/i.test(line));
      const matchedWeeks = weekLines.filter((line) => terms.some((term) => line.includes(term))).length;
      const requiredMatches = Math.max(2, Math.ceil(expectedWeeks * 0.5));
      if (matchedWeeks < requiredMatches) {
        issues.push(`Focus-area weighting is weak: "${focusArea}" should dominate at least ${requiredMatches} weeks.`);
      }
    }
  ...
  evaluateQuality(command, text, context = {}) {
    ...
    if (command === 'studyplan') {
      const studyPlanValidation = this.validateStudyPlanCompleteness(text, {
        ...
        certification: context.studyPlanCertification || '',
        focusArea: context.studyPlanFocusArea || ''
      });
  ...
  buildRefinementPrompt({
    ...
    const studyPlanRefinementRules = command === 'studyplan'
      ? [
        ...
        '- Keep plan realistic for provided hours-per-week and skill level.',
        '- Include at least 5 concrete checklist items in Final Exam Readiness Checklist.',
        '- Include Certification Alignment Notes with at least 3 concise bullets.',
        studyPlanCertification
          ? `- Explicitly align tasks and outcomes to "${studyPlanCertification}" exam philosophy.`
          : '- Explicitly align tasks and outcomes to the selected certification.',
        studyPlanFocusArea
  ...
  async generateCyberResponse({ command, userInput }) {
    ...
    try {
      ...
      if (command === 'redteam' || command === 'studyplan') {
        ...
        const recoveryLabel = command === 'studyplan'
          ? 'Study-plan refinement still low quality, attempting recovery pass'
          : 'Red-team refinement still low quality, attempting recovery pass';
        this.logger.info(recoveryLabel, {
          command,
